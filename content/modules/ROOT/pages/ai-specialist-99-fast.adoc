...
For this project, https://roboflow.com/[Roboflow, window=_blank] will be the primary tool used for managing and preparing data.
...
1. Navigate to the  https://universe.roboflow.com/luisarizmendi/hardhat-or-hat-mocktrain/browse?queryText=&pageSize=50&startingIndex=0&browseQuery=true[dataset's URL that you want to Fork in Roboflow Universe, window=_blank].
...
It is named 'MockTrain' because it contains only a few images to speed up training during the workshop. However, the resulting model will not be usable. A pre-trained model, trained on a more comprehensive dataset https://universe.roboflow.com/luisarizmendi/hardhat-or-hat/dataset/1["Hardhat or Hat" Dataset, window=_blank], will be provided as part of this workshop . 
...
OpenShift AI is an integrated platform that simplifies the development and deployment of AI workloads. We will use it to create and manage the https://jupyter.org/[Jupyter Notebooks, window=_blank] for experimentation and model training.
...
In production environments, training machine learning models is not as simple as running a script or experimenting in a notebook. A robust pipeline is essential. In this workshop we will use https://www.kubeflow.org/docs/components/pipelines/overview/[Kubeflow Pipelines, window=_blank].
...
5. Adjust the "Batch Size" and the "Epochs". Since you will be running a xref:ai-specialist-01-data.adoc#_mock_training_dataset[*Mock Training*] you can configure them to "1" to speed-up the pipeline execution.
...
4- Select "Deploy" on the top right corner
...
* Make deployed models available through an external route: `not selected`
...
When the Inference server appears as avialable in the https://rhods-dashboard-redhat-ods-applications.apps.{ocp_cluster_url}[OpenShift AI, window=_blank] console, you will be able to copy the internal endpoint that we need to use to access to it (copy the endpoint).
...
4- Change the `<CHANGE ME TO INFERENCE INTERNAL ENDPOINT>` string with the internal endpoint value that you copied from the deployed Inference Server

[NOTE]

If you used the values proposed in the workshop the `MODEL_NAME` environment variable will be `hardhat` so you can keep the default
...
* https://github.com/luisarizmendi/workshop-moving-ai-to-the-edge/blob/main/resources/solutions/ai-specialist/serving/apps/object-detection-stream-manager[Camera Stream Manager, window=_blank]

* Inference server

* https://github.com/luisarizmendi/workshop-moving-ai-to-the-edge/blob/main/resources/solutions/ai-specialist/serving/apps/object-detection-action[Actuator service, window=_blank]

* https://github.com/luisarizmendi/workshop-moving-ai-to-the-edge/blob/main/resources/solutions/ai-specialist/serving/apps/object-detection-dashboard/src/backend[Dashboard backend, window=_blank]

* https://github.com/luisarizmendi/workshop-moving-ai-to-the-edge/blob/main/resources/solutions/ai-specialist/serving/apps/object-detection-dashboard/src/frontend[Dashboard frontend, window=_blank]
...
[example]
====
Follow the steps below to create the https://github.com/luisarizmendi/workshop-moving-ai-to-the-edge/blob/main/resources/solutions/ai-specialist/serving/apps/object-detection-dashboard/src/backend[Dashboard backend, window=_blank] and https://github.com/luisarizmendi/workshop-moving-ai-to-the-edge/blob/main/resources/solutions/ai-specialist/serving/apps/object-detection-dashboard/src/frontend[Dashboard frontend, window=_blank] applications in OpenShift:
...
1- Choose the right https://github.com/luisarizmendi/workshop-moving-ai-to-the-edge/tree/main/resources/assets/hardhat-detection-apps/hardhat-detection-inference/inference_triton/deploy/podman[Inference Server deployment script, window=_blank] depending if you are using GPU or not
...
----
<your-script-name> create quay.io/luisarizmendi/modelcar-hardhat:v1
----

[NOTE]

Since you performed a "xref:ai-specialist-01-data.adoc#_mock_training_dataset[*Mock Training*]" you will be using the pre-generated modelcar Container image instead the one that you created.
...
[source,shell,role=execute,subs="attributes"]
----
sudo podman run -it --rm -p 5000:5000 --privileged --net host -e TRITON_SERVER_URL=localhost:8001 -e MODEL_NAME=hardhat -e CLASS_NAMES=helmet,no_helmet quay.io/luisarizmendi/object-detection-stream-manager:grpc
----
...
[NOTE]
====
During the container launch, you specify the class names. It's important to note that the order of these names matters. In the provided pre-trained model, there are two classes: `class0` corresponds to the detection of a hardhat, while `class1` corresponds to the detection of no hardhat. 

With the `CLASS_NAMES` variable you can define the names for those classes, for example you can set to "helmet,no_helmet" so the `class0` matches the `helmet` label. However, this configuration may vary depending on your specific model. The most reliable way to determine the correct class names and their order is by reviewing the output during deployment and testing of the inference server in OpenShift. This will show which class names are being detected by your model.
====
