...
3- Click "Create project" and give that new project a name (ie. `{user}-ai-test`)
...
* Model name: Change it to a meanful name. Do not keep the default since it could be a very long name that can prevent the inference server to run correctly, for example `hardhat-test`
...
If you want to see the deployment progress or check the Inference Server logs you can go to the https://console-openshift-console.apps.{ocp_cluster_url}[OpenShift Console] and check "Workloads > Pods". If you click on the Pod's name you will have access to the "Events" and "Logs" (the Pod is composed by four containers and one init container and you can select each of them to see their logs by using the dropdown menu that by default shows `kserve-container`).
