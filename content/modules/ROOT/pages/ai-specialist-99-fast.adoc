...
* Require token authentication: `not selected`v
...
When the Inference server appears as avialable in the https://rhods-dashboard-redhat-ods-applications.apps.{ocp_cluster_url}[OpenShift AI, window=_blank] console, you will be able to copy the endpoint that we need to use to access to it (copy the endpoint).
      ...
      - name: batch-model-api
        ...
        env:
          - name: TRITON_SERVER_URL
            value: '<CHANGE ME TO INFERENCE ENDPOINT>:8001'
          - name: MODEL_NAME
            value: 'hardhat'
---
apiVersion: v1
...
4- Change the `<CHANGE ME TO INFERENCE ENDPOINT>` string with the endpoint value that you copied from the deployed Inference Server
