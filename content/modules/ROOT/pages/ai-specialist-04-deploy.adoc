...
podman create \
  ...
  -c "mkdir -p /mnt/models && \
    skopeo copy --override-os linux docker://workshop-registry-quay-openshift-operators.apps.{ocp_cluster_url}/{user}/<INSERT YOUR MODEL:TAG> oci-archive:/tmp/modelcar.tar && \
    mkdir -p /tmp/image && \
