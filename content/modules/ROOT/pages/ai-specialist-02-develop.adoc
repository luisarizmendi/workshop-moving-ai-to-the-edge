...
A fix is required to address an issue in the workshop. Since we are not using valid SSL certificates, the default OpenShift Data Foundation object storage API is configured to use HTTPS. As a result, when you attempt to run the pipeline, you will encounter the following error:

image::ai-build-pipeline-error.png[]

To resolve this, open your Jupyter Environment, launch the Workbench, and navigate to "Runtimes" (represented by a gear icon in the left menu). Locate the "Cloud Object Storage Endpoint" setting and change `https` to `http`.
