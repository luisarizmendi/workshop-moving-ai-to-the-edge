...
5. Adjust the "Batch Size" and the "Epochs". If you are running a xref:ai-specialist-01-data.adoc#_mock_training_dataset[xref:ai-specialist-01-data.adoc#_mock_training_dataset[*Mock Training*] you can configure them to "1" to speed-up the pipeline execution.
...
When the Kubeflow Pipeline reaches the `create-modelcar` you can navigate to the "Pipelines" menu in the https://console-openshift-console.apps.{ocp_cluster_url}[OpenShift Console]. There you can see the OpenShift Pipeline execution:
...
In summary, this task generates a directory structure https://github.com/luisarizmendi/workshop-moving-ai-to-the-edge/tree/main/resources/solutions/ai-specialist/training/modelcar[like the one shown here ] consists of a `Containerfile` at the root level and a `models` folder that will contain the model itself.

[IMPORTANT]

Depending on the inference server you plan to use (covered in the next module), your model must follow specific requirements—such as file format, directory structure, and optional configuration files. In our example we will use https://developer.nvidia.com/blog/nvidia-triton-inference-server-boosts-deep-learning-inference/[NVIDIA Triton server] so we have our model in the `onnx` extension with the  `<model name>/<version>/model.onnx` directory structure and included the `config.pbtxt` inference server configuration file shown below.
...
The content is generated automatically by the Pipeline but be aware that the output value configured in this file is critical, as the Inference Server will fail to start if it doesn't match the model's expected output dimensions. In this example, the value is set to `[1, 6, 8400]`. This corresponds to a model with two classes (`hardhat` and `no-hardhat`). The second number (`6`) reflects the number of predicted values per anchor, which includes class scores and bounding box parameters. When you add more classes (as you'll do in the final module), you must increase this second value accordingly. For instance, if you add one additional class, the value should be updated to `[1, 7, 8400]`.



3- `build-and-push`: Performs the container image build using https://hub.tekton.dev/tekton/task/kaniko[Kaniko] and push it into the https://workshop-registry-quay-openshift-operators.apps.{ocp_cluster_url}[Quay Container Image Registry].
  ...
  steps:
    ...
    - computeResources: {}
      ...
      script: |
        set -e
        image="$(params.IMAGE)"
        echo -n "" | tee "$(results.IMAGE_URL.path)"
  workspaces:
...
If the image is not there check if the OpenShift Pipeline has already finished or if it is still building the image (in that case wait until it is completed).
...
The easiest way to use the https://huggingface.co/luisarizmendi/hardhat-or-hat[provided pre-trained model] in your environment is to pull https://quay.io/repository/luisarizmendi/modelcar-hardhat[the v1 of this pre-created modelcar image] and push it into the https://workshop-registry-quay-openshift-operators.apps.{ocp_cluster_url}[Quay Container Image Registry].

[NOTE]

You can also download the https://huggingface.co/luisarizmendi/hardhat-or-hat/tree/main/v1/model/onnx/1[version 1 (v1) of the model, more specifically the `onnx` version] and upload it into the the right location (`serving` directory) on the Object Storage using the https://s3-browser-{user}-ai-models-{user}-tools.apps.{ocp_cluster_url}[Object Storage browser], replacing the one that was created by the Kubeflow Pipeline, and then create the `modelcar` container image into the https://workshop-registry-quay-openshift-operators.apps.{ocp_cluster_url}[Quay Container Image Registry]by re-runing the OpenShift Pipeline after changing the file with the mode. Go to the Pipelines menu in the OpenShift Console, select the pipeline and move into the "PipelinesRun" tab, there you can click the three dots on the right and select "Rerun".
