...
The script creates a https://docs.podman.io/en/stable/markdown/podman-pod.1.html[Podman POD] with an `init-container` that pull the `modelcar` container, extract the model files and share them with the Inference Server. That's the same approach that when you are using the `modelcar` in OpenShift.
...
===== Deploy the Camera stream manager
...
----
 podman run -it --rm --network=host -e MONITORED_CLASSES=no_helmet -e ALERT_ENDPOINT=<DASHBOARD_BACKEND_OCP_ROUTE>/alert -e ALIVE_ENDPOINT=<DASHBOARD_BACKEND_OCP_ROUTE>/alive quay.io/luisarizmendi/object-detection-action:prod
----
...
Let’s examine the [.underline]*non-edge architecture* we’ll set up for testing. As you can see, the difference comparing it with the "pure" edge deployment is where the Inference Server is located.


image::ai-deploy-non-edge-arch.png[]

[example]
====
Let's deploy this architecture:

1- You should have the Inference Server already deployed, you don't need to deploy it locally, just take note of the inference URL again.


2- Deploy the "Camera Stream Manager". You can use the steps shown in the previous point, but with an small difference. You need to deploy the Camera Stream Manager indicating the URL endpoint for the Inference Server (by default it uses `localhost`). You can do it by using the `INFERENCE_SERVER_URL` environment variable (it will be something similar to `http://<NAME>-{user}-test.apps.{ocp_cluster_url}`).

----
sudo podman run -it --rm --net host -p 5000:5000 --privileged -e CLASS_NAMES=hardhat,no_helmet -e INFERENCE_SERVER_URL=INFERENCE_SERVER_URL=<INFERENCE_SERVER_OCP_ROUTE>/v2/models/hardhat/infer quay.io/luisarizmendi/object-detection-stream-manager:prod
----
