...
** Endpoint: Use the internal OpenShift Data Foundation endpoint for object storage, `s3.openshift-storage.svc` 
** Region: You can keep the Region empty but it's better to include any string (e.g. `local`).
...
Now there is fix that you need to apply. Due that in the workshop we are not using valid SSL Certificates, if you se the default configured OpenShift Data Foundation object storage API, that targets to https, you will get the following error when you try to run the pipeline:

image::ai-build-pipeline-error.png[]

In order to fix it you need to go into your Jupyter Environment, open the Workbench and navigate to "Runtimes" (represented by a gear icon in the left menu) and change the `https` to `http` in the "Cloud Object Storage Endpoint".
 
image::ai-build-pipeline-endpoint-config.png[]

[NOTE] 

The runtime configuration has been automatically loaded.If the Data Science Pipelines runtime configuration is empty try to restart your workbench once the pipeline server is active.
...
The Persistent Volume is not autogenerated, so you will need to manually create the Persistent Volume Claim with the specified name in the project where the pipeline will run ("{user}-ai")
