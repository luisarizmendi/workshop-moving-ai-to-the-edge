...
* Create a new connection:
  ...
  * Conection name: I suggest something like `quay-hosted`
...
----
sudo podman run -it --rm -p 5000:5000 --privileged --net host -e TRITON_SERVER_URL=<CHANGE ME TO INFERENCE EXTERNAL ENDPOINT>:8001 -e MODEL_NAME=hardhat -e CLASS_NAMES=helmet,no_helmet quay.io/luisarizmendi/object-detection-stream-manager:grpc
----
...
4- Look for the multiarch image that you want to "promote" and click on the gear icon on the right, then select "Add new tag"
