...
[CAUTION]
If you have multiple versions of your dataset, make sure you are using the correct version number under project.version. For example, if you created a new version as part of the "xref:ai-specialist-01-data.adoc#_mocktrain_dataset[Mock Training]" (training the model with a smaller dataset), verify that the version matches the intended dataset. 
...
Remember to use the "xref:ai-specialist-01-data.adoc#_mocktrain_dataset[Mock Training]" Dataset if you want to save time while trying this step.
...
Remember that if you want to save time you could use the "xref:ai-specialist-01-data.adoc#_mocktrain_dataset[Mock Training]" Dataset (although it produces a non-usable model).
